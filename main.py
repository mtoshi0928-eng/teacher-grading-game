...
# --- フォントの設定（ローカルWindows / Web自動判別・完全フォールバック） ---
def get_safe_font(size):
    # 1. フォルダ内に同梱フォント(ttf/otf)があれば最優先
    for font_file in ["NotoSansJP-Regular.ttf", "font.ttf", "meiryo.ttc"]:
        try:
            return pygame.font.Font(font_file, size)
        except:
            pass
    # 2. Windows / Mac / Linux の標準日本語フォントを探索
    system_fonts = ["msgothic", "meiryo", "yumincho", "hgyoukaishotai", "hg教科書体", "notosanscjkjp", "hiraginominchopron"]
...
try:
    chime_sound = pygame.mixer.Sound("Japanese_School_Bell02-02(Slow-Mid).mp3")
except:
    chime_sound = None
...
# =================================================================
# 非同期メインループ（WebAssembly / Pygbag 必須構造）
# =================================================================
async def main():
    ...
    while True:
        ...
        pygame.display.flip()
        await asyncio.sleep(0)  # ★Webブラウザ実行のための非同期スイッチ
